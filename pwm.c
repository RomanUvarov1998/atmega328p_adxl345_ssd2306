...
void pwm_init(void) {
    ...
    DDRD |= _BV(DDD3);
    PORTD &= ~_BV(PORTD3);
    TCCR2A |= _BV(COM2B1); // set on BOTTOM, clear on compare match OCR0B
    ...
    DDRD |= _BV(DDD5);
    PORTD &= ~_BV(PORTD5);
    TCCR0A |= _BV(COM0B1); // set on BOTTOM, clear on compare match OCR0B
    ...
    DDRD |= _BV(DDD6);
    PORTD &= ~_BV(PORTD6);
    TCCR0A |= _BV(COM0A1); // set on BOTTOM, clear on compare match OCR0B
